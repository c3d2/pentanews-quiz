...
function spOpenVideo(a, videoid){
	if($.browser.msie && $.browser.version.slice(0,1) <= 6) {
		...
		// oder in der aufrufenden Seite?
			...
			// oder per JS-Variable?
			else if (spOasSetPanoPlayerCategory != null) {
				hashQs='#oas.videobelegung=' + spOasSetPanoPlayerCategory;
			}
...
function spOasSetReminder(html){
	var adString = html[0].toLowerCase();
	if(!adString || adString == null || adString == 'null' || adString == ''){
		document.getElementById('spOasReminder').removeAttribute('class', 0);
		document.getElementById('spOasReminder').innerHTML = '';
	}else{
		if(adString.indexOf('<swf>') > -1 || adString.indexOf('<agencytrackingurl>') > -1){
			html = "";
			document.getElementById('spOasReminder').removeAttribute('class', 0);
			document.getElementById('spOasReminder').innerHTML = '';
		}else if(adString.indexOf('<mafourl>') > -1){
			var pi = adString.indexOf('<mafourl>' + 9);
			var s = e.substring(pi);
			var si = adString.indexOf('</mafourl>');
			var suffix = s.substring(si);
			var s = s.replace(suffix, '');
			document.getElementById('spOasReminder').removeAttribute('class', 0);
			document.getElementById('spOasReminder').innerHTML = 's';
		}else{
			document.getElementById('spOasReminder').className = "spCommercial";
...
function spStartVideo(videoId, belegung, credit, displaycat, videocat, server, trackingsize, startmod, onAdStart, onAdEnd, onVideoEnd, onFinalEnd, embedWidth, embedHeight){
	...
	if (spClientIsIDevice()){
		if (parseInt(videoId) >= 58671) {
			var spReplaceVideo = document.getElementById('spReplaceVideo');
			var videoNode = document.createElement('video');
			spReplaceVideo.appendChild(videoNode);
			videoNode.setAttribute("src", "/video/media/video-"+videoId+".html");
			videoNode.setAttribute("width", embedWidth);
			videoNode.setAttribute("height", embedHeight);
			videoNode.setAttribute("controls", "controls");
			if (startmod == "true") {
				videoNode.setAttribute("preload", "preload");
		...
		var url      = encodeURI(spStripHashParams(document.location.href));
		...
		swfobject.embedSWF("/static/flash/flashvideo/coreFlashPlayer-003.swf", "spReplaceVideo", embedWidth, embedHeight, "10.0.0", "", PlayerFlashVars, PlayerParams);
	} else {
		var spNoFlashPlayer = document.getElementById('spNoFlashPlayer');
		spNoFlashPlayer.setAttribute("style", "display:block;");
	}
